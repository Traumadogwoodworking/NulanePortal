...
`https://www.definian.com/signal` remains the embedded portal page. The onboarding QR deliberately uses the separate stable production route `https://vercel-portal-exact-traumadogwoodworkings-projects.vercel.app/definian/start`. That route renders the shared Facility Registration experience with Definian branding, an email-first Create account / Sign in choice, numbered setup instructions, and the verified iPhone and Android installation links. The selected action is handed to the fixed callback origin in a URL fragment that is never sent to the server and is removed before Auth0 starts. Both authentication actions then reuse the existing facility-registration Auth0 handoff with the Definian tenant, client, organization, and fixed callback; successful web authentication returns to the embedded Signal page. If a mobile browser drops callback query parameters, the preserved onboarding transaction immediately creates a fresh request for the original signup or login action instead of showing the generic callback recovery screen.
