...
describe("Power BI dashboard selection", () => {
  test("uses the clean AWCT display name while preserving the backend organization key", () => {
    const branding = resolvePortalBranding({
      session: buildSession("another.user@gmail.com"),
      pathname: "/home",
    });

    expect(branding.normalizedKey).toBe("awct.inc");
    expect(branding.organizationName).toBe("AWCT");
  });
