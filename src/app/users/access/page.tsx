...
export default function AccessPage() {
  ...
  const [locationMemberships, setLocationMemberships] = useState<LocationMembership[]>([]);
  ...
  const loadAccessData = useCallback(async () => {
    ...
    try {
      ...
      setLocationMemberships(locationMembers);
      setFacilities(facilityList);
      setSelectedRoleKey((current) => current || roleList[0]?.key || "");
      setSelectedUserId((current) => (current && userList.some((user) => user.id === current) ? current : userList[0]?.id ?? null));
    } catch (error) {
      setStatusMessage(error instanceof Error ? error.message : "Unable to load access data.");
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredMemberships = useMemo(() => {
    return memberships.filter((membership) => {
      if (selectedRoleKey && membership.role !== selectedRoleKey) return false;
      if (facilityFilter !== FACILITY_ALL) {
        return locationMemberships.some(
          (m) => m.user_id === membership.user_id && m.location_id === facilityFilter
        );
      }
      return true;
    });
  }, [memberships, selectedRoleKey, facilityFilter, locationMemberships]);

  const selectedRole = roles.find((role) => role.key === selectedRoleKey) ?? roles[0] ?? null;

  const selectedUser = (selectedUserId && userLookup[selectedUserId]) || null;

  const selectedUserLocations = useMemo(() => {
    if (!selectedUser) return [];
    return locationMemberships
      .filter((m) => m.user_id === selectedUser.id)
      .map((m) => facilityLookup[m.location_id])
      .filter(Boolean) as FacilitySummary[];
  }, [selectedUser, locationMemberships, facilityLookup]);

  const impactedUsers = useMemo(() => {
    return filteredMemberships
      .map((m) => userLookup[m.user_id])
      .filter(Boolean) as UserSummary[];
  }, [filteredMemberships, userLookup]);
    ...
    <TooltipProvider>
      <article className="space-y-4">
        <PageSection
          ...
          description="Registry of roles, permissions, and active deployments."
          variant="panel"
          actions={
          ...
          <div className="grid min-w-0 gap-4 lg:grid-cols-4">
            <div className="min-w-0 lg:col-span-3">
               {isLoading ? (
                 ...
                 <EmptyState title="Registry Empty" description="No roles configured." />
               ) : (
                  <div className="-mx-4 -mb-4">
                    <DataTableShell columns={["Identity Group", "Scope", "Permissions", "Registry Status"]}>
                      {roles.map((role) => {
                        ...
                        return (
                          <tr
                            ...
                            className={`group transition-all duration-150 cursor-pointer ${
                              isSelected ? "bg-slate-100 dark:bg-slate-900/40" : "hover:bg-slate-50 dark:hover:bg-slate-900/40"
                            }`}
                          >
                            <td className="px-4 py-2">
                               <div className="flex items-center gap-3">
                                  <div className={`w-7 h-7 rounded bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 ${isSelected ? 'text-slate-900 dark:text-white' : ''}`}>
                            ...
                            <td className="px-4 py-2">
                              <StatusBadge label={role.scope} tone="neutral" />
                            </td>
                            <td className="px-4 py-2">
                            ...
                            <td className="px-4 py-2">
                              <StatusBadge label={role.status} tone={role.status === "active" ? "positive" : "danger"} />
                            </td>
                          </tr>
                        );
                      })}
                      ...
                      <div className="space-y-4">
                        <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
                          <div className="flex items-start gap-2.5">
                             <div className="w-7 h-7 rounded-lg bg-white dark:bg-slate-900 flex items-center justify-center text-slate-900 shadow-sm">
                          ...
                          <div className="mt-3 flex gap-1">
                             ...
                             <StatusBadge label={selectedUser.isActive ? "Online" : "Offline"} tone={selectedUser.isActive ? "positive" : "danger"} />
                          </div>
                        </div>
